...
- **2026-09-20 本机 Build28**：刷新改为固定单按钮内切换转圈；已存在 DSH 根目录时补建空 skills 子目录，实机 DeepSeek Harness 已找到。完整回归与包检查通过，中央内容保持、临时旧 App 已清理。公开仍 Build27；本轮没有发布。Xcode 许可未接受，使用独立 CLT 配套测试框架完成验证，命令见 scratch/dsh-fix/tool-bin/swift。见 [记录](reviews/2026-09-20-refresh-dsh.md)。

- **2026-09-20 v0.2.6 Build27 已上线并本机更新**：最终复核补修预览后目标变化仍沿用旧确认的问题。631 Swift 实际通过/1 跳过、28 Python 通过，完整发行检查通过；Latest、标签、5 个公开附件和更新源核对一致。本机5份Skill保留，168个资料文件无新增缺失，临时旧App清理；关于页实测已是最新。详情长文CUA断线的视觉/返回缺口仍保留，不再重复安装；后续用户反馈另行处理。见 [本次发布记录](reviews/2026-09-20-release-026.md)。以下为历史记录。

- **2026-09-20 当前本机 Build 26**：受管变化副本可以比较并确认替换，复用原有事务恢复，确认许可不持久复用。ai-news WorkBuddy 实际仅 .DS_Store 不同，现已排除这类安装误报并实机显示已安装、能正常操作；6 个原文件含系统文件均未改。631 Swift 实际通过/1 跳过，28 Python 通过，打包检查通过，未发布。长文本详情的 CUA 验收缺口仍保留。见 [记录](reviews/2026-09-20-managed-replacement.md)。


- **2026-09-20 最新本机 Build 24，安装记录校正已实机通过**：github-readme-design GPT 的实际内容早已等于当前中央版本，旧部署记录造成误拦。已加入真实完整比对后的记录校正，不写 Skill 文件。629 Swift 实际通过、1 跳过，28 Python 通过，正式列表已验证为已安装，文件修改时间亦保持。未公开发布。详见 [记录](reviews/2026-09-20-matching-installation.md)。前一切片详情窗口的 CUA 断线验收缺口仍保留。


- **2026-09-20 当前本机 Build 23，正文对比已实现，前台验收待补**：连续双栏删除线/黄色文字背景与安装确认右上角只读详情已接入，627 Swift 实际通过、1 公网跳过、28 Python 通过，release/隐私检查通过。真实启动成功，Skill 正文及安装关系无本轮操作；点击更新时 CUA native pipe closed，重连/重置无效。工具恢复后补“我的 Skills 查看更新”和“安装确认 → 查看更新 → 关闭回原确认”的视觉、滚动、返回验证，不重新安装或重复创建备份。临时旧 App 已清理，公开仍 Build 20。详见 [记录](reviews/2026-09-20-diff-integration.md)。
