...
- **阶段**：v0.2.1 已正式发布并完成 GitHub／本地开发／本地安装三端运行内容对齐
- **最近在做**：修复空分类在主列表被隐藏、却出现在“移动到”菜单的“幽灵分类”问题；升级到 `0.2.1 (4)`，完成公开 Release、附件回读、正式安装和用户数据保持验收
- **下一步**：如需补齐陌生用户安装证据，在另一台干净 Mac 通过浏览器下载 v0.2.1，走一次“隐私与安全 → 仍要打开”并完成核心鼠标流程
...
- 当前产品决策明确采用 ad-hoc Hardened Runtime DMG，不做 Developer ID 签名与 Apple 公证；Gatekeeper 阻止并要求用户走“仍要打开”属于预期安装路径。v0.2.1 的 278 项测试、DMG、包内隐私、图标、来源清单、SHA-256、GitHub Release 下载回读和本机正式启动均已通过。
