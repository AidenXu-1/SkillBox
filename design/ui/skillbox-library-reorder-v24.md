# SkillBox v24：实时让位式 Skill 排序候选

## 结论

v23 的插入线虽然克制，但仍让用户用一条抽象的线猜测落位，并且现有 SwiftUI 状态缺少可靠的“拖动结束”总出口。v24 改成消费级列表常用的实时让位模式：被拖动卡片浮起跟手，目标位置让出一个完整卡片高度的空位，系统蓝线只在空位边界辅助定位。
...
2. 卡片浮到列表上方并贴着指针移动，不旋转、不跳位。
3. 指针越过相邻卡片中线时，周围卡片用约 `180 ms` 无回弹动画让出一个完整空位。
4. 空位底边显示 `2 pt` 系统蓝线；没有圆点、文字胶囊或常驻提示。
...
- 指针离开有效列表区域时，空位和蓝线立即消失；浮起卡片继续跟手，松手则回原位。
...
- 可操作原型：`skillbox-library-reorder-v24.html`
- 打开后默认展示“拖动中”，可用顶部按钮查看普通、松手落位、取消拖动和右键菜单。
- 在“普通列表”状态下，可直接按住任意卡片上下拖动；按 `Esc` 可验证取消。
- 静态预览：`skillbox-library-reorder-v24-drag.png`
